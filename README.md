...
- **Mini-EMR** (no auth, per the brief): [`/admin`](https://zealthy-emr-portal.vercel.app/admin) — open the patients table directly
...
The `/admin` surface is the operator-facing EMR. Per the exercise brief it requires
**no authentication** (a real EMR would, of course). It's built entirely on the Phase-3
...
The **mini-EMR at `/admin`** requires no login — open it directly (per the brief).
